...
behavior. Results show \emph{[placeholder for measured speedup]} on an
NVIDIA RTX~3060 and illustrate the implementation cost of cache-side-channel
resistance on commodity GPUs.
\end{abstract}
...
\section{Results}
[Insert seven plots: throughput vs depth (AES and Spongent), GPU vs CPU
speedup at $d{=}20$, AES kernel-variant comparison, AES vs Spongent on GPU,
memory-mode trade-off, per-level kernel-launch overhead.]

[Insert table summarizing throughput at $d{=}20$ across all backends.]

[Discussion paragraph: AES-vs-Spongent gap; GPU-vs-CPU speedup magnitude;
T-tables vs S-box throughput; memory-mode trade-offs.]
